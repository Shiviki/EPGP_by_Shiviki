...
local sstScrollCols = {
       ...
       { ["name"] = "Мейн/Офф",		    ["width"] = 210, ["align"] = "LEFT", 	  ["defaultsort"] = "desc", ["sort"] = "desc", ["color"] = {["r"] = 0.25, ["g"] = 1.00, ["b"] = 0.25, ["a"] = 1.0 }, ["sortnext"]=10 }, --,
       ...
       { ["name"] = "Ставка",		        ["width"] = 60,  ["align"] = "RIGHT"},
...
function LootMasterML:GetFrame()
	...
	lblGPOverride:SetText( "Начислить ГП:" );
	...
	btnDiscard:SetText("Отмена аукциона")
...
function LootMasterML:ShowRaidInfoLookup()
    ...
    if not frame then
        ...
        local titletext = titleFrame:CreateFontString(nil,"OVERLAY","GameFontNormalSmall")
        titletext:SetPoint("CENTER",titleFrame,"CENTER",0,1)
        titletext:SetText( string.format("EPGPLootMaster by Beoc for Not So Bad - Scourge x2, Sirus.su", LootMaster:GetVersionString() ) )
        frame.titleFrame = titleFrame
        --#endregion
