...
function LootMaster:ShowUpdateFrame( sender, iVersion, sVersion )
    ...
    message( string.format("Автонапоминалка от %s: пожалуйста обновите epgp_lootmaster с дискорда Not So Bad. Если вы этого не сделаете, можете не получить лут в рейде..", sender, sVersion ) )
end
...
function LootMaster:InitUI()
    ...
    titletext:SetText( string.format("EPGPLootMaster by Beoc for Not So Bad - Scourge x2, Sirus.su", self:GetVersionString() ) )    
...
function LootMaster:SaveNote( frame, keepVisibility )
    frame.data.note = gsub(frame.tbNote:GetText() or '', '%^', '');
        ...
        frame.lblNote:SetText(format('Моя ставка: %s', frame.data.note or ''))
    else
...
function LootMaster:CreateLootSelectFrame()
    ...
    b:SetScript("OnUpdate", function( o, elapsed )
        ...
        frame.lblTimeout:SetText( format("%s до конца ставок", ceil(frame.data.timeoutLeft)) )
    ...
    btnNoteSave:SetWidth(75)
    btnNoteSave:SetText("Ставка")
...
function LootMaster:UpdateLootUI()
    ...
    for i, data in ipairs(self.lootList) do repeat
        ...
        lootFrame.lblInfo:SetText(format("ilevel: %s", data.ilevel or -1, gp))
...
function LootMaster:VersionActionClick(name)
    ...
    if self.versioncheckframe.rows[rowID].cols[3].value == 0 then
        SendChatMessage("Привет: Установи аддон EPGP из дискорда ги Not So Bad", "WHISPER", nil, name);
    end
end
